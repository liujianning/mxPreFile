...
@interface SRSpeedLaughView ()

@property (nonatomic, strong) SRNetworkSpeedMonitor *speedMonitor;

@end

@implementation SRSpeedLaughView
...
- (void)initialize {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(networkSpeedChanged:) name:SRDownloadNetworkSpeedNotificationKey object:nil];
}

- (void)dealloc {
    [self.speedMonitor stopNetworkSpeedMonitor];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:SRDownloadNetworkSpeedNotificationKey object:nil];
}
...
- (void)networkSpeedChanged:(NSNotification *)sender {
    NSString *downloadSpped = [sender.userInfo objectForKey:SRNetworkSpeedNotificationKey];
...
- (SRNetworkSpeedMonitor *)speedMonitor {
    if (!_speedMonitor) {
        _speedMonitor = [[SRNetworkSpeedMonitor alloc] init];
    }
    return _speedMonitor;
}

- (SRLoadingView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[SRLoadingView alloc] init];
